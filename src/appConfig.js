...
const joiEnvSchema = Joi.object()
  .keys({
    ...
    MEMBER_URLS: Joi.string().required().custom((value, helpers) => {
      // convert comma separated list of MEMBERS to array lists
      const membersArray = `${value}`
      .split(',')
      .map((eachAddress) => `${eachAddress}`.trim())
      return membersArray
    ...
    DNS_PROVIDER: Joi.string().description('DNS_PROVIDER').default('cloudflare').valid(...['cloudflare','technitium']),
  ...
  .custom((obj, helpers) => {
    ...
    if (!MEMBER_URLS?.includes(SELF_URL)) {
      return helpers.message(`SELF_URL(${SELF_URL}) needs to be part of MEMBER_URLS(${MEMBER_URLS})`)
    }
    return obj
  })
